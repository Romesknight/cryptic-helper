...
import { parseAnnotation, type AnnotationSegment } from '@/lib/claude/parse-response';
 ...
 */
export default function AnnotatedClue({ annotation, className }: AnnotatedClueProps) {
...
function AnnotationSpan({ segment }: { segment: AnnotationSegment }) {
  switch (segment.type) {
    ...
    case 'explanation':
      return <span className="annotation-explanation">({segment.content})</span>;
    default:
