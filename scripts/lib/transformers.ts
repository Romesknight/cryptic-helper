 ...
 */
function getPrimaryType(indicators: IndicatorInfo[]): EigenfooIndicatorType | null {
...
function buildAnnotation(
  ...
  primaryType: EigenfooIndicatorType
): string {
...
export function transformRow(row: EigenfooRawRow): TransformResult {
  // Sanitize fields
  const answer = sanitizeText(row.answer).toUpperCase().replace(/[^A-Z]/g, '');
  ...
  const clueTextClean = clueText.replace(/\s*\(\d+(?:[,-]\d+)*\)\s*$/, '').trim();
  ...
  return {
    clue: {
      ...
      difficulty: calcDifficulty(answer, clueTypeSlug, clueTextClean, indicators.length),
      source,
      is_verified: true,
    },
    error: null,
